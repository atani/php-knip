...
- **PHPバージョン対応**
  - PHP 4.4 〜 PHP 8.3（PHP 4コードの解析に対応、段階的バージョンアップを支援）
...
Options:
  ...
  -o, --output=OUTPUT          出力ファイルパス（デフォルト: 標準出力）
      ...
      --php-version=VERSION    パース用PHPバージョン（4.4, 5.6, 7.0, ..., 8.3）
      --no-colors              カラー出力を無効化
      --no-cache               キャッシュを無効化
...
- PHP 4.4以上のコードを解析対象にできます（実行にはPHP 5.6以上が必要）
